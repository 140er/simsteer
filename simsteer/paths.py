...
an extraction directory that is wiped on exit. Writes there are lost. So:
  ...
  - Writable state (per-game calibration, liveparams, livecalib_state, profile
...
This module is package-root infrastructure: it depends on nothing else in
`simsteer`, so `core`, `runtime`, `games`, and `app` may all import it freely.
"""
...
def bundle_dir() -> Path:
    ...
    Dev:    the repo root (parent of the `simsteer/` package).
    """
...
def asset_path(name: str) -> Path:
    ...
    May not exist — callers (e.g. `simsteer.app.audio`) should handle missing files."""
